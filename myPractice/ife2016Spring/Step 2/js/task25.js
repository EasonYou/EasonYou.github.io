window.onload=function(){
	...
	document.getElementById("cont").addEventListener("click", function(event){
        traversalTree.switch.call(traversalTree,event.target);
    },false);
}
...
TraversalTree.prototype.switch=function(target){
	if(target.tagName!=='A'){
		return;
	}
	if(target.flag===undefined){
		target.flag=true;
	}
	console.log(target.flag);
	target.flag=!target.flag;
	var child=target.parentNode.getElementsByTagName('*');
	if(!target.flag){
		for(var i=1;i<child.length;i++){
			child[i].style.display='none';
			child[0].style.backgroundColor='#aeaeae';
		}
	}else{
		for(var i=1;i<child.length;i++){
			child[i].style.display='inline-block';
			child[0].style.backgroundColor='#000';
		}
	}
}
TraversalTree.prototype.focus=function(target){
	if(target.className==='container'||target.className==='switch'){
		return;
	}
...
TraversalTree.prototype.create=function(){
	...
	var nodeVal=this.target.innerHTML;
	if(this.target.children.length===0){
		this.target.innerHTML='<a href="#" class="switch"></a>'+nodeVal+'<div class="box">'+text+"</div>";
	}else{
